...
字符串常量用**双引号**包裹，如`"s"`，其本质是一个地址，指向由`'s'`、`'\0'`组成的字符串中的第一个字符`'s'`，即字符串常量**第一个字符所在的内存地址**。
...
第一行语句让`fruit`成为了新类型的名称，于是在第二行我们可以声明一个`fruit`类型的枚举变量。同时，`apple`、`orange`、`pinapple`成为了`fruit`类型的符号常量，称为**枚举量**，默认对应整数值0~2。

声明的时候可以**略掉**`fruit`，这样**只会创建一些枚举量（常量）**，而不会产生新类型。

```c++
enum {apple, orange, pinapple};
int color = apple + orange;
```

> 不止`fruit`类型的枚举变量可以用这些枚举量，这个作用域下都可以。
>
> ```c++
> enum fruit {apple, orange, pinapple};
> int color = apple;
...
虽然这些`fruit`类型的枚举量（`orange`、`apple`等）实际上对应的是一个数字，但是我们并不能给`good`赋一个`int`类型的变量。这是因为**枚举量可以被自动提升为`int`类型，但是`int`类型不能自动转换为枚举类型**。

```c++
good = 0;//invalid, 虽然apple对应0
...
```c++
fruit good = apple + orange;//invalid
```

这是因为运算后**有可能会超出运算范围**。比如`orange`是1，`pinapple`是2，一加变成3了，超出去了。
...
不是说好的没有定义运算么，怎么这又行了呢？

可以这么理解：前面说枚举量可提升为`int`，在算术表达式中被转换成了`int`。由于`int`不能自动转换成枚举，所以`fruit good = apple + orange;`是不行的，但是将`int`赋给`int`是再正常不过得了，所以`int color = apple + orange;`是可以的。

不信的话，我们引入类型强制转换，这就可以了：

```c++
good = fruit(apple + orange);
```
...
```c++
enum {apple = 18, orange = 22, pinapple= 38};
```

也可以**只指定部分**，如：

```c++
enum {apple, orange = 22, pinapple};
```

这种情况下，`apple`仍然是0，`pinapple`会比它前面的`orange`大1。
...
上面的例子中，`zero`和`null`都是0，`one`和`numero_uno`都是1。

## 枚举的取值范围

最初对于枚举而言，只有声明时指出的那些值才是有效的。但是C++通过类型强转，增加了可赋给枚举变量的值。虽说增加了，但还是**有范围**的。

范围上界的确定：声明中的最大值用二进制表示要几位，那么范围的上界便是这些二进制位所能表示的最大值（全是1），所以通常是2的幂次方减1。

范围下界的确认：如果声明中的最小值是正数，那么下界便是0。如果是负数，那么下界的确认便和上面关于上界的确认大致相同，只不过是再添一个负号罢了。

选择用多少空间存储枚举值，是由编译器确定的。

> 接下来便是指针的内容了，我准备开一个`4.5`章阅读笔记，专门记录下。今天是2023年的1月4日。时间过得好快。
